...
class CandidatePlotDefaultLayout(CandidatePlotLayout):
    ...
    @property
    def axPeriodCurve(self):
        return plt.subplot(self.grid[4:7, 7:])
    ...
    @property
    def axACCCurve(self):
         return plt.subplot(self.grid[7:, 7:])

    @property
    def axWidthCurve(self):
        return plt.subplot(self.grid[7:, 3:7])
...
class CandidatePlot(object):
    ...
    def __init__(self, cand,accurve,beam,ver, layout='default', figsize=None, dpi=None):
        self.cand = cand
	self.beam = beam
	self.ver = ver
        self.layout = None
    ...
    def _plotACCCurve(self):
        ax = self.layout.axACCCurve
        #cv = self.cand.width_curve

	snr = self.accurve[1]
        
	trial = self.accurve[0]
        

        # Duty cycle trials expressed in percent
        #ducy_trials = cv.trials / (width / ducy) * 100.0

        ax.plot(trial[np.where(snr>0)[0]], snr[np.where(snr>0)[0]], '--go')
        plt.ylim(0, snr.max()*1.30)
        plt.xlim(-97,97)
	#plt.xscale('log')
        plt.grid(which='minor', linestyle=':')
        plt.grid(which='major', linestyle='-')
        plt.xlabel('Acceration $m \cdot s^{-2}$')
        plt.ylabel("S/N")
	...
	a = self.accurve[0][np.where(self.accurve[1]==self.accurve[1].max())][0]
            #print i
	table.add_entry(Entry("Acceration", "{:.1f}".format(a), "$m \cdot s^{-2}$"))
        snr = self.cand.metadata['best_snr']
        table.add_entry(Entry("S/N", "{:.1f}".format(snr)))
	table.add_entry(Entry("Beam", self.beam))
	table.add_entry(Entry("Ver.", self.ver))
