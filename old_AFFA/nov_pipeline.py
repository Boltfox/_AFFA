...
import time
os.system("python /home1/til/FFA-Acceration-pipeline/plot_candidate.py")
...
def pipenoacc(nfilpath,setting,fil,confighome,beam):
...
def pipe(nfilpath,setting,fil,confighome,beam,ver,step,al):
	...
	para=best(his)
	ripfold(nfilpath,para,hismap,al,k,his,beam,ver)
...
def ripfold2(nfilpath,para,hismap,al,k,his,beam,ver):
	...
	#		os.rename(nfilpath+"/"+hismap[para[i][0]][para[i][1]][para[i][2]],nfilpath+"/Resultisnrpda_"+str(snr)+"_"+str(p)+"_"+str(d)+"_"+str(a)+".h5")
			plt.close()
...
def main():
	...
	archiving=setting['mode']['archiving']
	accsearch=setting['mode']['accsearch']
	print("donwsamping")
	os.system("python "+confighome+"plot_candidate.py")
	...
	if accsearch == 'yes':
		...
		pipenoacc(nfilpath,setting,fil,confighome,Beam)
